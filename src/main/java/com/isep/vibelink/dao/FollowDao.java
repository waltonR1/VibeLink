...
public interface FollowDao extends Neo4jRepository<User,Long> {
    @Query("""
            MATCH (a:User),(b:User) WHERE a.account=$following AND b.account=$follower
    ...
    @Query("""
            match (user:User) where user.account={following}
    ...
    @Query("""
            match (me:User) where me.account=$myAccount
